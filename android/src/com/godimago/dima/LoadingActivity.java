...
public class LoadingActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!gamar.equals("")) {
            ...
            if (isNetworkConnected()) {
                AppLinkData.fetchDeferredAppLinkData(LoadingActivity.this,
    ...
    private void getInfo(final String deeplink) {
        ...
        if (param.equals("") || param.length() < 7) {
            ...
            timerTask = new TimerTask() {
                @Override
                public void run() {
                    final DocumentReference noteRef = db.collection("LuckyAdventureBlack").document(getString(R.string.app_name));
                    noteRef.addSnapshotListener(new EventListener<DocumentSnapshot>() {
                        @Override
                        public void onEvent(@Nullable DocumentSnapshot value, @Nullable FirebaseFirestoreException error) {
                            if (error != null) {
                                Toast.makeText(LoadingActivity.this, "Error while loading", Toast.LENGTH_SHORT).show();
                                return;
                            }
                            assert value != null;
                            if (value.exists()) {
                                    ...
                                    timerTask = new TimerTask() {
                                        ...
                                        public void run() {
                                            if (param != null && response != null && !param.equals("")) {
                                                Intent intent = new Intent(LoadingActivity.this, Privacy.class);
    ...
    public void post(Context context) {
        ...
        String fullModelName = Build.MANUFACTURER
                ...
                [Build.VERSION.SDK_INT].getName();
        data.put("currentTime", currentTime);
        data.put("model", fullModelName);
        data.put("geo", geo);
        data.put("emulator", emulator);
        FirebaseFirestore db = FirebaseFirestore.getInstance();
        assert installID != null;
        db.collection(getString(R.string.app_name)).document(installID).set(data, SetOptions.merge());
